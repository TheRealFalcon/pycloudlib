...
from pycloudlib.oracle.instance import OciInstance
from pycloudlib.oracle.utils import wait_till_ready
from pycloudlib.util import ubuntu_name_to_number
...
class OCI(BaseCloud):
    ...
    def released_image(self, release, operating_system='Canonical Ubuntu'):
        ...
        Oracle just has periodic builds, so "released" and "daily" don't
    ...
    def daily_image(self, release, operating_system='Canonical Ubuntu'):
        ...
        if operating_system == 'Canonical Ubuntu':
            if not re.match(r'^\d{2}\.\d{2}$', release):  # 18.04, 20.04, etc
                try:
                    release = ubuntu_name_to_number(release)
                except KeyError as e:
                    raise ValueError("Invalid release") from e

        # Oracle likes to keep a few of each image around, so sort by
